...
export default function DocumentChatPage({ params }: { params: { document_id: string } }) {
  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    try {
      const response = await chat(userMsg.content, [params.document_id]);
    ...
    <div className="flex h-screen bg-white">
      ...
      <div className="flex-1 flex flex-col relative">
        <header className="border-b px-6 py-4 flex items-center justify-between">
          <h2 className="font-semibold text-lg">Document Specific Chat</h2>
        </header>
